...
class PelvicTiltActivity : AppCompatActivity(), SensorEventListener {
    ...
    private fun startTimer() {
        timer = object : CountDownTimer(userTimeThreshold, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                timerValue = millisUntilFinished

                // Increment tracker on each second
                tracker++
                updateUI()
            }

            override fun onFinish() {
                overallCount++ // Increment overall count once the full time threshold is reached
                updateUI()

                if (overallCount >= userCountThreshold) {
                    mediaPlayer.start() // Play sound when userCountThreshold is reached
                    overallCount = 0 // Reset overall count
                }
            }
        }
        timer.start()
